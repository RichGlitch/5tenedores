...
export default function InfoUser(props){
    ...
    const changeAvatar =async () => {
        ...
        if(resultPermissionCamera === "denied"){
            toastRef.current.show('nel no hay permisos');
        }else{
                ...
                if(result.cancelled){
                    toastRef.current.show('se canceló');
                }else{
    ...
    const updatePhotoUrl = uid => {
        firebase
            ...
            .catch(()=>{
                toastRef.current.show('error al recuperar avatar');
            });
    };
    ...
    return(
        <View style={styles.viewUserInfo}>
            <Avatar
                ...
                source={{uri: photoURL ? photoURL : "http://api.adorable.io/avatars/172/"+{uid}+".png"}}
            />
            <View >
